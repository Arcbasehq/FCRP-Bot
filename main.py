...
def get_department_regex(member: discord.Member):
...
async def validate_member(member: discord.Member):
    ...
    dept_regex = get_department_regex(member)

    if dept_regex:
        if not dept_regex.search(name):
            try:
                await member.send(f"Your callsign `{member.display_name}` is invalid for your department. You were removed from the voice channel.")
    ...
    if RANKED_REGEX.search(name):
        if member.voice and member.voice.channel and member.voice.channel.id == VOICE_CHANNEL_ID:
            try:
                await member.send("You are using a ranked callsign without a LEO role. You were removed from the voice channel.")
            except discord.Forbidden:
                pass
            try:
                await member.move_to(None)
        ...
        if member.voice and member.voice.channel and member.voice.channel.id == VOICE_CHANNEL_ID:
            try:
                await member.send("Your callsign is invalid. Allowed formats: `9D-##` or `8D-##`.")
            except discord.Forbidden:
                pass
            try:
                await member.move_to(None)
